...
- `main` 为受保护分支，管理员同样不得绕过保护规则直接提交或推送。
- 每个问题或改动必须从最新 `main` 创建一个独立分支；默认使用 `codex/<topic>` 命名。
- 所有改动必须通过 PR 合并到 `main`，不得以任何形式直接合入。
- GitHub 仓库必须开启「PR 合并后自动删除源分支」。
- PR 合并后必须完成本地收尾：切回 `main`、以 fast-forward 方式同步、执行 `git fetch --prune`，并删除已合并的本地源分支。
- 工作区存在无关改动时，只暂存本次任务文件，不得把其他改动带入分支、commit 或 PR。
