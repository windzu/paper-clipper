...
- `title`：完整标题。
- `paper_status`：论文阅读状态，取值为 `To Read` / `Reading` / `Done`。使用领域专属字段名，避免与 Blog、Social 等内容的 `status` 状态机混用。
- `category`：研究方向，例如 `End2End`、`BEV`、`Occupancy`。
- `publish_date`：论文发布时间。
- `abstract`：摘要，用于快速判断是否值得读。

Base 中的 `properties.*.displayName` 只用于表头显示。视图的 `order` 和 `sort` 使用 Obsidian 保存的 note-property shorthand，例如 `short_title`、`paper_status` 和 `publish_date`；`filters` 使用 `note.paper_status` 等完整表达式。两者都不能引用 `Short Title`、`Paper Status` 等显示名。
...
- 字段：`short_title`, `file.name`, `title`, `paper_status`, `category`, `publish_date`, `abstract`
- 排序：`publish_date` 降序

### To Read

- 筛选：`paper_status == "To Read"`
- 字段：`short_title`, `file.name`, `title`, `category`, `publish_date`, `abstract`
- 排序：`publish_date` 降序

### Reading

- 筛选：`paper_status == "Reading"`
- 字段：`short_title`, `file.name`, `title`, `category`, `publish_date`, `abstract`

### Done

- 筛选：`paper_status == "Done"`
- 字段：`short_title`, `file.name`, `title`, `category`, `publish_date`, `abstract`
- 排序：`publish_date` 降序
...
- 默认视图聚焦 `short_title`、`title`、`paper_status`、`category`、`publish_date`、`abstract`，并保留 `file.name` 作为打开 note 的入口。
