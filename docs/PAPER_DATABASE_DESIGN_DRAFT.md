...
Base 中的 `properties.*.displayName` 只用于表头显示。视图的 `order`、`sort` 和 `filters` 必须引用真实 property identifier，例如 `note.short_title`、`note.paper_status` 和 `note.publish_date`，不能引用 `Short Title`、`Paper Status` 等显示名。
